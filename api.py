...
#     p = Presentation(theme='APSA-light', width=1600, height=900, scale=0.6).cl('default')
    p = Presentation(theme=Theme(),
                     width=1600,
                     height=900,
                     scale=0.2,
                     autoSlide=5000,
                     mathjax='local').cl('default')
    p.align('left')
    p.color('#fff')
    print p.dict_css
#     quit()
...
#===============================================================================
#     page1 = Page()
#     page1.addColumn(attr_width="60%", float='left')
#     page1.addColumn(attr_width="40%", float='right')
#
#     page1.Columns[0].par('One must evaluate the robustness of a randomized algorithm,\
#         i.e., the degree to which the outputs of separate identical runs agree.'
#         )
#     page1.Columns[0].par('edges that alter the global characteristics of the network\
#          and obfuscate the most important structures therein. Graph presentation_pruning_channing is the process of\
#          identifying the most significant edges according to a generative null model, '
#          )
#     page1.Columns[1].par('edges that alter the global characteristics of the network\
#         and obfuscate the most important structures therein. Graph presentation_pruning_channing is the process of\
#         identifying the most.'
#         )
#
#     page1.Columns[1].addImage(src='http://www.image.png',
#                               attr_height="200px")
    ...
    #===========================================================================
    page0 = FrontPage()
    page0.title('Statistical Significance In Weighted Complex Networks')
    a = page0.author('Navid Dianati')
    a.institution('The Lazer Lab Northeastern University')
    a.institution('IQSS Harvard University')

    page0.author('Elaine F. Stranahan').institution('Harvard University')
    p.addPage(page0)
    #===============================================================================
    # Cover
    #===============================================================================

    page11 = p.page(trans='fade')
    page11.header('This is another page')
    page11.par('Very often, we need to deal with weighted networks where the weight \
        of an edge linking two vertices counts the number of events or indicates the \
        strength of the tie between the two. If this network is too dense,')
    page11.video('http://www.naviddianati.com/wp-content/uploads/2015/04/frac-bethe1.mp4?_=1').w('50%')

    #===========================================================================.color('red')
    # page 1
    #===========================================================================

    page1 = Page(trans="fade")
    page1.j(1).w("48%")
    page1.j(1).par(' Here is the list of the things I like:')
    list2 = page1.j(1).list()
    list2.item('There is nothing in here.')
    list2.item("Most of the things you see here aren't actually here.")
    list2.item("But there is actually 'something' here which is not a thing.")


    page1.j(2).w('48%').par("fdsafdsfd fds fds fds fdsa fdsa fdsa fdsa fds fds")
    list1 = page1.j(2).list()
    list1.item('This is the content of item1').fr()
    list1.item('This is the <b>content</b> of item2 which is another item')
    list1.item('This is the content of item3 which is yet another item')
    p.addPage(page1)


    pageA1 = PageSection('Theory', trans="fade-in")
    p.addPage(pageA1)
    pageA2 = PageSection('Introduction')
    pageA3 = PageSection('What is The Problem?')
    p.addPage(pageA3)

    #===========================================================================
    # page2
    #===========================================================================
    page2 = Page(trans="fade", background="")
    page2.header('This is the title of the page')
    page2.j(1).w('50%')
    page2.j(2).w('50%')
    page2.j(1).par('This is the shit').cl('h2yellow').cl('fragment')
    page2.j(1).par("Again, we assume as the null model, that each of the T directed edges\
          must choose a source vertex and a target vertex <b>independently</b> at random, such that\
          both the in-degree distribution and the out-degree").cl('fragment')
    page2.i(2).par(r'$\begin{align*}\
        & \Gamma(r,t)=\mathscr{L}_{\vec{r},t}^{\dagger}\left\{ c(T-t)\exp\left[\frac{\pi r^{2}}{c(T-t)}\right]\right\}\
        \end{align*}$')

    page2.j(2).im('file:///home/navid/Dropbox/Presentations/FEC/files/Ohio-sub-2.png').fr().width('100%')
    p.addPage(page2)

    #==========================================================================
    # page 3
    #==========================================================================
    page3 = Page(trans="fade", color="#fff", background="")
    page3.i(1).fr()
    page3.i(2)
    page3.i(1).j(1).par('Where are you now?').cl('white')
    page3.i(1).j(2).im('http://www.naviddianati.com/papers/graphpruning/html/graph-measures-airports-crop.png', classes=['large']).w('80%')
    page3.i(2).j(1).par('Where are you now?')
    page3.i(2).j(1).par('Where are you now?')
    p.addPage(page3)
    ...
    # page 4
    #===========================================================================
    page4 = Page('Why not both?', color="#fff")
    page4.par('<b>Then we do this:</b>').cl('fragment')
    page4.par('Very often, we need to deal with weighted networks where the')
    page4.c(1).w('50%').video('/home/navid/Dropbox/graphs/networks/output.mp4')
    page4.c(2).w('50%').par('Very often, we need to deal with weighted networks \
        where the weight of an edge linking two vertices counts  ').align('left')
    p.addPage(page4)


    page5 = Page(background='')
    page5.i(1).im('/home/navid/Dropbox/graphs/FEC/newyork-occupation-edited-light-background.png').h('100%')
    p.addPage(page5)
    ...
    #
    #===========================================================================
    page = p.page('Testing centered text')
    page.par('This paragraph number 1').center().fr()
    page.par('This paragraph number 2').center().fr()
    page.par('This paragraph number 3').center().fr()
    #===========================================================================
    #
    #===========================================================================
    page6 = p.pagesection('Thank You!')
    page6.h4('Navid Dianati').color('white')
#     page6.im('/home/navid/Dropbox/identity/QR-name-email.png').w('25%')
...
#         print line
    ...
    p.export('presentation-test.html')
